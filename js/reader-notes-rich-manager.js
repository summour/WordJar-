// WordJar Reader Notes Rich Manager V1
// Complete Notes-style manager: selectable list, note actions, rich editor, custom highlight colors, and rich Reader handoff.

(function installReaderNotesRichManager() {
  if (window.__wordjarReaderNotesRichManagerInstalled) return;
  window.__wordjarReaderNotesRichManagerInstalled = true;
  ...
  function injectStyles() {
    if (document.getElementById('readerNotesRichManagerStyle')) return;
    const style = document.createElement('style');
    style.id = 'readerNotesRichManagerStyle';
    style.textContent = `
      ...
      .rn-editor { flex:1 1 auto; min-height:0; display:flex; flex-direction:column; padding:0 20px 116px; }
      ...
      .rn-tools { pointer-events:auto; display:flex; align-items:center; gap:13px; padding:10px 13px; border:1px solid var(--bdr); border-radius:18px; background:var(--sur); box-shadow:0 16px 34px rgba(0,0,0,.10); max-width:calc(100% - 68px); overflow-x:auto; scrollbar-width:none; }
      .rn-tools::-webkit-scrollbar { display:none; }
      .rn-tool { min-width:26px; border:0; background:transparent; color:var(--ink); padding:0; display:inline-flex; align-items:center; justify-content:center; font-size:22px; font-weight:950; line-height:1; }
      .rn-tool svg { width:24px; height:24px; stroke-width:2.4; }
      .rn-color { width:28px; height:28px; min-width:28px; padding:0; border:2px solid var(--bdr); border-radius:999px; overflow:hidden; background:transparent; }
      .rn-color input { width:36px; height:36px; border:0; padding:0; margin:-4px; background:transparent; }
      .rn-swatch { width:24px; height:24px; min-width:24px; border:1px solid var(--bdr); border-radius:999px; }
  ...
  function renderEditor(id = '') {
    ...
    shell(`
      ...
      <div class="rn-bottom">
        <div class="rn-tools" onmousedown="saveReaderNoteSelection()" ontouchstart="saveReaderNoteSelection()">
          <button class="rn-tool" type="button" onclick="formatReaderNote('bold')" aria-label="Bold">B</button>
          <button class="rn-tool" type="button" onclick="formatReaderNote('italic')" aria-label="Italic"><i>I</i></button>
          <button class="rn-tool" type="button" onclick="formatReaderNote('underline')" aria-label="Underline">${svg('underline')}</button>
          <button class="rn-tool" type="button" onclick="formatReaderNote('strike')" aria-label="Strike">${svg('strike')}</button>
          <button class="rn-tool" type="button" onclick="formatReaderNote('highlight')" aria-label="Highlight">${svg('highlight')}</button>
          ${swatches.map(c => `<button class="rn-swatch" type="button" style="background:${c}" onclick="setReaderHighlightColor('${c}'); formatReaderNote('highlight')" aria-label="Highlight ${c}"></button>`).join('')}
          <label class="rn-color" aria-label="Custom highlight color"><input id="rnColorPicker" type="color" value="${currentHighlightColor}" onchange="setReaderHighlightColor(this.value); formatReaderNote('highlight')"></label>
          <button class="rn-tool" type="button" onclick="formatReaderNote('bullet')" aria-label="Bullet List">${svg('bullet')}</button>
          <button class="rn-tool" type="button" onclick="formatReaderNote('number')" aria-label="Numbered List">${svg('number')}</button>
          <button class="rn-tool" type="button" onclick="insertReaderNoteChecklistIOS()" aria-label="Checklist">${svg('checklist')}</button>
          <button class="rn-tool" type="button" onclick="formatReaderNote('outdent')" aria-label="Outdent">${svg('outdent')}</button>
          <button class="rn-tool" type="button" onclick="formatReaderNote('indent')" aria-label="Indent">${svg('indent')}</button>
          <button class="rn-tool" type="button" onclick="createReaderNoteLink()" aria-label="Link">${svg('link')}</button>
          <button class="rn-tool" type="button" onclick="clearReaderNoteFormatting()" aria-label="Clear Formatting">Aa</button>
    ...
    const editor = document.getElementById('rnEditorBody');
    if (editor) {
      ['keyup','mouseup','touchend','selectionchange'].forEach(evt => editor.addEventListener(evt, saveSelectionRange));
  ...
  function clearRichModeOnManualEdit() {
    ...
    if (!input || input.__wordjarRichClearBound) return;
    input.addEventListener('input', () => {
      D.reader.isRichNote = false;
      D.reader.richHTML = '';
    });
    input.__wordjarRichClearBound = true;
  }
  ...
  window.openReaderNote = function openReaderNote(id) {
    ...
    closeO('readerNotesModal');
    if (typeof renderReader === 'function') renderReader();
    toast('Opened rich note in Reader');
  };
  ...
  window.saveCurrentReaderNoteAsNew = function saveCurrentReaderNoteAsNewManager() {
    ...
    const n = { id:'rn' + Date.now() + '-' + Math.random().toString(36).slice(2,6), title:titleFrom(text), text, html, createdAt:nowLabel(), updatedAt:nowLabel() };
    D.readerNotes.push(n);
    D.reader.activeNoteId = n.id;
    save();
  ...
  window.renderReader = function renderReaderWithRichNotes() {
    ...
    const tokens = document.getElementById('readerTokens');
    if (tokens && !tokens.__richClickBound) {
